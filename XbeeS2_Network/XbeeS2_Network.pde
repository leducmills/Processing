...
String version = "1.1";
int numSensors = 2; //number of sensor values for each Xbee
...
void draw() {
  ...
  if (data.value1 >=0 && data.address != null) { 
    ...
    data.value1 = map(data.value1, 0, 1023, 0, 255);
    data.value2 = map(data.value2, 0, 1023, 0, 255);
...
//   float temperatureCelsius = map(data.value, 0, 1023, 1, height);
//
